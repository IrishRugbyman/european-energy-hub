...
import pandas as pd
from loaders._base import get_read_conn  # type: ignore[import]
...
def build_lng_tables() -> dict:
    ...
    df = pd.DataFrame(
        rows, columns=["gas_day", "country", "inventory_gwh", "sendout_gwh", "dtmi_gwh", "dtrs_gwh"]
    )
    ...
    eu = df.groupby("gas_day", as_index=False).agg(
    ...
    df["sendout_util_pct"] = (
        df["sendout_gwh"] / df["dtrs_gwh"].replace(0, float("nan")) * 100
    ).round(2)
    ...
    for country, gas_day in latest_dates.items():
        ...
        row["d7_sendout_gwh"] = (
            (row["sendout_gwh"] - d7_sendout)
            if (d7_sendout is not None and row["sendout_gwh"] is not None)
            else None
        )
        ...
        row["vs_avg5_sendout"] = (
            (row["sendout_gwh"] - avg5)
            if (avg5 is not None and row["sendout_gwh"] is not None)
            else None
        )
    ...
    eu_recent["avg5_sendout"] = eu_recent["doy"].map(
        lambda d: eu_seas.loc[d, "avg5_sendout"] if d in eu_seas.index else None
    )
