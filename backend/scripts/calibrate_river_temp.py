...
def main() -> None:
    """Calibrate the river-temperature heuristic against Hub'Eau observations."""
    ...
    print(
        f"\nRiver data: {len(df_river)} days, {df_river['date'].min()} to {df_river['date'].max()}"
    )
    ...
    o = df["offset_c"]
    print(
        f"  n={len(o):,}  mean={o.mean():.2f}  "
        f"p5={o.quantile(0.05):.1f}  p25={o.quantile(0.25):.1f}  "
        f"p50={o.quantile(0.50):.1f}  p75={o.quantile(0.75):.1f}  "
        f"p95={o.quantile(0.95):.1f}\n"
    )
    ...
    for m, grp in df.groupby("month"):
        o = grp["offset_c"]
        print(
            f"{month_names[m]:>6} {len(o):>5} {o.mean():>6.2f} "
            f"{o.quantile(0.25):>6.1f} {o.quantile(0.50):>6.1f} "
            f"{o.quantile(0.75):>6.1f} {o.quantile(0.95):>6.1f}"
        )
    ...
    print(
        f"{'Regime':>8} {'n':>5} {'mean':>6} {'p10':>6} {'p25':>6} {'p50':>6} {'p75':>6} {'p90':>6}"
    )
    for lbl, grp in df.groupby("regime", observed=True):
        ...
        o = grp["offset_c"]
        print(
            f"{str(lbl):>8} {len(o):>5} {o.mean():>6.2f} "
            f"{o.quantile(0.10):>6.1f} {o.quantile(0.25):>6.1f} "
            f"{o.quantile(0.50):>6.1f} {o.quantile(0.75):>6.1f} "
            f"{o.quantile(0.90):>6.1f}"
        )
    ...
    if not hot.empty:
        ...
        for day, grp in hot.groupby("day_in_spell"):
            ...
            if len(o) < 3:
                continue
            print(
                f"{day:>4} {len(o):>5} {o.mean():>6.2f} "
                f"{o.quantile(0.25):>6.1f} {o.quantile(0.50):>6.1f} "
                f"{o.quantile(0.75):>6.1f}"
            )
    ...
    for thr in [22, 24, 25, 26, 27, 28]:
        ...
        avg_air = (
            summer[summer["river_max_c"] >= thr]["air_max_c"].mean() if n > 0 else float("nan")
        )
        print(
            f"  river >= {thr}C: {n:4d} days ({pct:.1f}%), "
    ...
    if not hot_summer.empty:
        ...
        print(
            f"     median offset = {off.median():.2f}C  "
            f"(p25={off.quantile(0.25):.2f}, p75={off.quantile(0.75):.2f})"
        )
        print(
            f"     -> for air_max=35C: "
            f"median river={35 + off.median():.1f}, "
            f"p25={35 + off.quantile(0.25):.1f}, "
            f"p75={35 + off.quantile(0.75):.1f}"
        )
        print(
            f"     -> for air_max=38C: "
            f"median river={38 + off.median():.1f}, "
            f"p25={38 + off.quantile(0.25):.1f}, "
            f"p75={38 + off.quantile(0.75):.1f}"
        )
