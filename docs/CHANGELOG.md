...
## 2026-06-28 - Phase 85: Hydrological balance as a price fundamental on /spreads

**Built:** New cross-market analysis quantifying the Nordic "hydrological balance" price fundamental. `analytics/hydro_balance.py` joins the weekly reservoir balance (storage vs 5yr same-week seasonal norm, in %) against each country's weekly mean day-ahead price (cross-zone mean of its bidding zones), then fits price ~ balance per country and reports the Pearson correlation, a week-over-week differenced correlation (`corr_diff`, trend-removed), R², OLS slope (EUR/MWh per pp of fill), current standing, fitted price, and residual. Covers 11 hydro-reporting countries (NO SE FI ES PT FR IT AT CH RO GR) with both reservoir and price coverage. Two precomputed energy_hub.duckdb tables (`hydro_price_balance_summary`, `hydro_price_balance_series`, ~3yr weekly), `GET /api/hydro/price-balance` endpoint, and `HydroBalanceSection` on the /spreads "Market Structure" tab: country selector, 4 stat cards (corr + detrended, current balance, price-vs-fit residual, slope), a weekly balance-vs-price scatter with the OLS regression line, and a cross-country ranking table with both the levels and differenced correlation, classifying each market as hydro-set / hydro-influenced / thermal-set. 135 backend tests (+1), 60 frontend tests.

**Findings:** The levels correlation grades how hydro-set each market looks: IT (-0.69, R²=0.48), PT (-0.62), ES (-0.60) most reservoir-sensitive; NO (-0.47) the classic Nordic signal; thermal/nuclear markets near zero (AT -0.01, FR -0.09, CH +0.05). But the differenced correlation is the more honest read, and it reshuffles the ranking: IT collapses from -0.69 to -0.06 once detrended (its apparent link was almost entirely the shared post-crisis trend of refilling reservoirs alongside falling prices), while ES (-0.23), PT (-0.22), and notably FR (-0.22, despite a near-zero levels corr) retain a genuine week-to-week hydro response. This is a textbook spurious-trend correction: the slow common drift inflates levels correlations, and first-differencing isolates the real contemporaneous signal. As of mid-June 2026 Norway's reservoir balance is -2.6% (price 59 EUR/MWh, 3.6 below the hydro-fitted level) while Spain sits +38.5% (brimming reservoirs, price 46 EUR/MWh, 16 above fit). This completes the supply-side picture on /spreads, which was previously DE-LU thermal-only: the hydro-dominated Nordic/Iberian/Italian markets now have their dominant fundamental made explicit, with the trend confound called out rather than papered over.

## 2026-06-28 - Navigation and /spreads information architecture polish

**Built:** Front-of-house UX cleanup across the whole app. Nav in `__root.tsx` regrouped into EU and US clusters with a visual divider; a real responsive mobile nav sheet (hamburger toggle, two-column region-grouped grid) replaces the old icon-only horizontal scroll that hid labels on small screens. Added a styled `notFoundComponent` (404 page with a back-to-home link) so unknown URLs no longer render blank. New `/map` route that redirects to `/power` so the documented unified-dashboard URL and any stale bookmarks resolve instead of 404-ing. The `/spreads` page (which had grown to ~30 stacked analytics sections) is now organized into 5 tabs (Spreads, Fundamental Model, Validation, Portfolio & Signal, Market Structure) using conditional rendering so off-screen recharts containers stay unmounted and never measure a zero-size box. Landing-page market-pulse cards get equal-height flex treatment and brighter label contrast; the FR nuclear pill on /power moves below the nav so it no longer overlaps the new mobile menu. Removed an unused `@heroicons/react` dependency.

**Findings:** /spreads had accumulated 30+ analytics sections in a single scroll from the P42-83 signal-research arc - the tab split makes the page navigable without removing any content. All 60 frontend vitest cases pass and the production build is clean.

## 2026-06-28 - Phase 84: Hydro reservoir levels on /power

**Built:** End-to-end hydro reservoir feature across all layers. PostgreSQL: `hydro_reservoir_weekly` table with week_date, country, stored_twh, capacity_twh; fetcher `entso_e_hydro.py` pulling ENTSO-E A72 (reservoir filling) for 12 hydro-significant EU countries (NO, SE, AT, CH, FI, PT, ES, HR, RO, HU, SK, CZ). Analytics: `analytics/hydro.py` computing latest snapshot with vs-5yr-avg %, YoY %, and 5yr-percentile-rank per country; seasonal band (p10/p25/p75/p90) and history series for country drill-down. refresh.py: hydro tables written to energy_hub.duckdb. API: `GET /api/hydro/map` (12-country choropleth payload) and `GET /api/hydro/country/{code}` (history + seasonal bands). Frontend: `hydro_fill` MapMetric mode on /power with a blue-green choropleth (dark blue = very full, grey = no data); `HydroReservoirSection` in UnifiedZonePanel showing stored TWh, vs-average %, YoY %, 5yr rank, and a 52-week seasonal fan chart (5-year min-max band + 5yr avg line + prior year + current year). 8-task SDD execution across PostgreSQL, analytics, API, frontend types, scale, map mode, panel, and deploy.

**Findings:** Norway (52.7 TWh) at -2.6% vs 5yr avg and -5.7% YoY as of 2026-06-14, 20th percentile of last 5 years - notably below average for the season. Sweden (-8.1%, -20.3% YoY) and the Alpine countries (AT, CH) tracking near or below seasonal norms. The hydro fill metric completes the renewable supply picture on /power: combined with the generation mix (fuel breakdown) and renewable % choropleth, users can now assess whether low wind/solar is being compensated by hydro drawdown or vice versa.
...
**Found:** AIC-justified on 4 of 5 fundamental zones: DE-LU (ΔAIC=-14.56, coef=+2.26 EUR/MWh per GW miss, CV=0.226), NL (ΔAIC=-8.59, coef=+3.41, CV=0.155), IT-NORD (ΔAIC=-8.07, coef=+2.85, CV=0.138), BE (ΔAIC=-2.83, coef=+6.59, CV=0.162). FR is not justified (ΔAIC=+0.22): nuclear baseload dominates FR price residuals and demand surprises add no channel. The coefficient is positive everywhere (higher demand miss = higher prices next day) and the stability (CV 0.14-0.23) is the best in the factor arc - significantly better than storage (CV 1.05-6.3) or nuclear*wind interaction. Current D-1 load error for DE-LU = +2.46 GW (actual demand higher than DA forecast yesterday, consistent with heat wave surge).

**Decision:** The demand-side signal is genuine and stable. The economic mechanism: when the DA market underestimates load, the thermal dispatch stack runs deeper than anticipated and the weather/industrial pattern persists 1-2 days. For gas-marginal zones, this pushes prices up via the gas CCGT merit order. For nuclear-heavy FR, the merit order is dominated by nuclear availability rather than marginal thermal, so load errors don't transmit to prices via the same channel. This is the strongest factor justified by AIC since the enriched model (P48). Current snapshot: DE-LU residual +33.7 EUR/MWh with a +2.46 GW demand miss - both consistent with the ongoing heat wave driving demand above model expectations.

**Artifacts:** `backend/analytics/fundamental.py` (`_fetch_fundamental_features_with_load_err`, `compute_load_error_factor_test`), `backend/app/schemas.py` (`LoadErrorScatterPoint`, `LoadErrorRollingPoint`, `LoadErrorFactorTestResponse`), `backend/app/main.py` (GET `/api/spreads/load-error-factor-test`), `backend/tests/test_endpoints.py` (`test_spreads_load_error_factor_test`), `frontend/src/lib/api.ts`, `frontend/src/routes/spreads.tsx` (`LoadErrorFactorSection`). 115 tests passing.

## 2026-06-28 - Phase 66: EU gas storage deviation as a fundamental factor on /spreads

**Tried:** Walk-forward AIC test of whether EU gas storage fill% vs 5yr seasonal average (storage_deviation = fill% - DOY-matched avg5 from storage_seasonal) adds information to the enriched nonlinear OLS fair-value model (P48+P54, 12 terms) beyond what TTF spot already captures. New `_fetch_fundamental_features_with_storage()` joins `storage_history` + `storage_seasonal` on DOY to compute the daily EU storage deviation, passed as a 13th column in `compute_storage_factor_test()`. Walk-forward at each day t: both the enriched baseline and the extended model (+storage_dev) are refit on [0..t-1], OOS residual compared on RMSE, Sharpe, and AIC.

**Found:** TTF fully impounds storage information for gas-anchored zones: DE-LU (ΔAIC=+1.38, r=0.012), NL (ΔAIC=+1.74, r=-0.067), BE (ΔAIC=+0.75, r=0.062), IT-NORD (ΔAIC=+1.51, r=-0.017) - all not AIC-justified and with near-zero Pearson correlations. France is the exception: ΔAIC=-2.92 (justified, threshold<-2), Pearson r=0.239, CV=1.05 (borderline stable). Economic mechanism: in nuclear-heavy FR, gas inventory scarcity creates asymmetric price risk when storage is low AND nuclear is curtailed (heat wave/river cooling limit). This scarcity-nuclear interaction is not fully priced in TTF spot at the zonal level. Current EU storage: -10.87pp vs 5yr average (historically tight); FR enriched residual today +33.7 EUR/MWh (partly nuclear heat risk, partly storage scarcity overlay in FR).

**Decision:** The finding is zone-specific and honest: for TTF-anchor zones, storage adds nothing (TTF is efficient). For FR, storage is a genuine marginal predictor but borderline (AIC yes, BIC disagrees at +1.04). The section ships as a documented, falsifiable test - the asymmetric zone-response is the key insight. Frontend: `StorageFactorSection` with zone picker, AIC verdict banner, 4-stat grid, coefficient stability panel, rolling 60d correlation chart, and storage_dev vs residual scatter. Defaults to FR. 114 tests passing.

**Artifacts:** `backend/analytics/fundamental.py` (`_fetch_fundamental_features_with_storage`, `compute_storage_factor_test`), `backend/app/schemas.py` (`StorageFactorStats`, `StorageFactorImprovementStats`, `StorageFactorCoef`, `StorageFactorRollingPoint`, `StorageFactorScatterPoint`, `StorageFactorTestResponse`), `backend/app/main.py` (GET `/api/spreads/storage-factor-test`), `backend/tests/test_endpoints.py` (`test_spreads_storage_factor_test`), `frontend/src/lib/api.ts` (types + `spreadsStorageFactorTest`), `frontend/src/routes/spreads.tsx` (`StorageFactorSection`).

## 2026-06-28 - Phase 65: reBAP vs DA z-score cross-signal correlation on /imbalance

**Built:** `GET /api/imbalance/rebap-zscore-correlation` and `RebapZscoreCorrelationSection` on /imbalance. Quantifies the cross-market linkage between the DA fundamental signal (DE-LU nonlinear OLS z-score) and the German RT balancing market (reBAP daily mean). Both signals are driven by the same underlying factor - renewable output shortfall - but from different market perspectives: DA prices in the shortfall at gate closure, reBAP prices it in real-time. The function runs the walk-forward nonlinear OLS on DE-LU (same computation as the nonlinear backtest arc), converts residuals to rolling z-scores, and joins with `imbalance_daily.mean_eur`. Reports overall Pearson and Spearman correlation, 60-day rolling Pearson series, raw scatter, and a z-score quintile dose-response. Cached at TTL=3600s. Frontend: dose-response bar chart, rolling correlation line chart, scatter plot, and live explanatory text.

**Key finding:** Pearson r=0.42 overall; dose-response from Q1 (z < -0.62: mean reBAP 88 EUR/MWh) to Q5 (z > 0.82: mean reBAP 115 EUR/MWh) - a 27 EUR/MWh differential showing a near-monotonic relationship between DA overpricing and RT balancing cost. Rolling 60d correlation currently 0.78 (elevated by the June 2026 nuclear heat wave: FR Rhone curtailment drives both DA prices above fair value and additional RT balancing pressure). The Spearman r=0.27 < Pearson r=0.42 indicates the correlation is driven partly by extreme episodes rather than a uniform rank relationship. 113 tests passing.

## 2026-06-28 - Phase 64: signal posture synthesis endpoint + UI section on /spreads

**Built:** `GET /api/spreads/signal-posture` and `SignalPostureSection` on /spreads. Synthesises the full research arc (P42-P63) into a concrete daily trading posture for each of the 5 fundamental zones. Logic: in drought regime (wind_pct < LOW_WIND_KNOT_PCT=8%), the nonlinear fade loses its edge (P56 walk-forward), so the zone posture flips to TREND; outside drought, the z-score drives STRONG FADE (|z| >= 1.0, full position), WEAK FADE (|z| >= 0.5, half position), or NEUTRAL (below noise threshold). Each zone card also shows the P60 optimal holding period k and best net Sharpe. A nuclear caveat is attached to FR when Rhone plants are at critical heat constraint (capacity_critical_mw > 0), noting the asymmetric fade risk if a heat wave persists. A systematic-alignment banner fires when 4+ zones agree (all FADE or all TREND), flagging a common factor event (weather, nuclear outage). Backend: `compute_signal_posture()` in `fundamental.py`, `ZonePostureRow` + `SignalPostureResponse` schemas, endpoint cached at 300s. Also added `wind_pct` to the fundamental model series output (was missing). Frontend: 5-column zone card grid with posture badge, z-score + pct-rank, wind%, drought flag, optimal k, and caveat. 112 tests passing.

**Live posture on 2026-06-28:** DE-LU STRONG FADE SHORT (z=+1.02, wind=9%, k=5); FR TREND (wind=7.8% - drought, nuclear caveat); NL WEAK FADE SHORT (z=+0.66, wind=10.8%); IT-NORD TREND (wind=0.1% - deep drought); BE WEAK FADE SHORT (z=+0.89, wind=10.7%). The split-regime picture (2 drought zones, 3 fadeable) reflects today's heterogeneous wind pattern: northern North Sea well-supplied, southern continental + Italy wind calm.

## 2026-06-28 - Phase 63: in-process TTL cache for expensive OLS endpoints

**Built:** Thread-safe in-process TTL cache (`_API_CACHE`, `_cached_compute()`) in `main.py`. Without caching, every visit to /spreads triggered fresh walk-forward OLS model fits on each request. Measured cold latencies: portfolio-backtest 1.2s, nonlinear-backtest 200ms, holding-period 100ms per zone. After the first visitor warms each key, subsequent requests serve in 9-25ms from the in-process dict. TTL=3600s for analytics endpoints (data refreshes twice daily, so 1h cache is safe); TTL=300s for signal-snapshot and market-pulse signal (tighter freshness for the live signal strip). Cache keys are endpoint-specific; per-zone endpoints use zone-qualified keys (e.g. `nl_backtest_FR`, `holding_period_DE-LU`) to avoid cross-zone collisions. Implementation uses `threading.Lock` around reads and writes; the expensive computation happens outside the lock so concurrent requests don't block each other waiting for the compute to finish. Cached: nonlinear-backtest (per zone), nonlinear-cost-robustness (per zone), nonlinear-edge-by-zone, regime-aware-backtest (per zone), portfolio-backtest, residual-mean-reversion, holding-period (per zone), signal-snapshot, market-pulse OLS signal. 111 tests passing.
...
**Key finding:** DE-LU net Sharpe rises from 1.12 at k=1 to 3.11 at k=5, then falls. FR rises monotonically to 6.56 at k=10. The signal identifies persistent overvaluation / undervaluation episodes that last longer than the OU half-life (which measures the average reversion rate of individual residuals, not the duration of multi-day episodes). Longer holding amortises cost aggressively, and for slow-reverting nuclear-baseload zones (FR), the signal has not fully decayed even at k=10. For wind-heavy zones the curve peaks earlier (NL, BE optimal around k=3-5). The section validates the P53 OU half-life result from a different angle: the Sharpe peak shifts with zone mean-reversion speed.

## 2026-06-28 - Phase 59: fuel cost decomposition chart on /spreads

**Built:** `FuelCostDecompositionChart` on /spreads - a new panel between the CSS/CDS/FSS time series and the multi-zone section. Two toggle modes: (1) "Marginal cost" - three line series: DA power (white), CCGT_MC = TTF/0.49 + EUA*0.364 (cyan dashed), coal_MC = API2/0.36 + EUA*0.96 (amber dashed). The gap between the power line and each cost line is exactly CSS and CDS - the chart makes it visually clear which fuel has the thinner margin. (2) "Components" - splits each MC into fuel cost and carbon (EUA) cost separately: gas fuel (cyan), gas CO2 (blue), coal fuel (amber), coal carbon (orange). A "today snapshot" strip shows the current numerical breakdown for both fuels with the derived CSS/CDS cross-check. Fully frontend - uses the existing `SpreadsDailyPoint` series already fetched for the time-series chart. No backend changes. Constants (GAS_EFF=0.49, GAS_EF=0.364, COAL_EFF=0.36, COAL_EF=0.96) are identical to the spark-dark research project.

**Decision:** The CSS/CDS time series shows the *spread* but not *why* it moved - was it TTF up, EUA up, or power up? The fuel cost decomposition makes that decomposition visual. The "components" mode specifically shows how the EUA price (which grew 15x from 2018 to 2022) progressively squeezed fuel plant economics even when fuel costs were flat, and how the 2022 energy crisis produced enormous spreads even at 200+ EUR/MWh TTF because power prices rose faster. The chart also makes the fuel-switching logic concrete: when the two dashed MC lines are close together (FSS near zero), a small EUA or TTF move flips the regime; when they're far apart, the regime is stable. 110 tests unchanged.
...
**Found:** The hypothesis was directionally wrong on the mechanism but the data still reveals structure. The interaction coefficient is NEGATIVE (not positive) on nuclear-heavy zones, meaning the extra price crash occurs when BOTH nuclear AND wind are HIGH together, not when nuclear is low + wind is high. On DE-LU and IT-NORD the coefficient is exactly 0 (Germany exited nuclear April 2023; Italy has no nuclear) - the channel is physically absent, as expected. On FR (coefficient -2.66, cv=0.39): ΔAIC = +0.99 (not justified), and the interaction HURTS the Sharpe (1.947 -> 1.681) because a tighter fair value absorbs the mean-reverting residual the fade trades. On BE (coefficient -29.6, cv=0.29 - reasonably stable): AIC is borderline justified (ΔAIC -3.43 < -2) and Sharpe IMPROVES (2.633 -> 2.918), but BIC disagrees (ΔBIC +0.53). On NL (coefficient -203, cv=0.22 but suspicious magnitude): AIC-justified (ΔAIC -3.61) but Sharpe drops (2.601 -> 2.532); the large coefficient points to numerical sensitivity when nuclear_lag1_gw is near-zero for most of the NL OOS window.

**Decision:** The enriched model (P48+P54) is the right canonical signal. The interaction fails on 4 of 5 zones by the AIC test, and even where it passes (BE), the BIC disagrees and the economic mechanism is the opposite of the original hypothesis. The section is shipped as a documented, falsifiable test of the hypothesis rather than a model to adopt: the verdict is honest (not justified), the coefficient sign is explained (high-nuclear + high-wind oversupply is the actual driver), and the per-zone table shows why some zones can't even be tested (zero nuclear). The section sits between EnrichedModelSection and GbmModelSection so the argument arc reads: enriched factors -> interaction test -> ML learner, each step asking "does adding complexity help?"

**Artifacts:** `backend/analytics/fundamental.py` (`compute_nuclear_wind_interaction`), `backend/app/schemas.py` (`NuclearWindInteractionResponse`, `NuclearWindCoef`), `backend/app/main.py` (GET `/api/spreads/nuclear-wind-interaction`), `backend/tests/test_endpoints.py` (`test_spreads_nuclear_wind_interaction`), `frontend/src/lib/api.ts` (types + `spreadsNuclearWindInteraction`), `frontend/src/routes/spreads.tsx` (`NuclearWindInteractionSection`). 99 tests passing.

## 2026-06-28 - Phase 55: reBAP renewable forecast-error signal on /imbalance

**Tried:** Added a walk-forward OLS signal to the /imbalance page predicting whether the next day's German reBAP will be above or below its 5-day trailing mean. Features (all D-1 gate-closure): D-1 wind forecast error, D-1 solar forecast error, D DA wind forecast (%-of-load), D-1 deviation of reBAP from its own 5-day mean. Train window 252 days, 1-day OOS, 2 EUR/MWh round-trip cost. Initial attempt used raw reBAP level as target with naive always-long comparison - that showed Sharpe 40 and 99.2% accuracy, flagged immediately as artefact of the structurally positive reBAP (positive 99.6% of days in OOS window). Revised target: reBAP *excess above trend*, where the naive is 50/50 and can be compared fairly.

**Found:** Model accuracy on excess direction: **70.3%** vs 50.5% naive baseline. OOS net Sharpe **3.75**, cumulative net 15,150 EUR/MWh over 664 days, max DD -1,178 EUR. The naive always-long baseline earns Sharpe 39.6 but this is structural: any strategy that's always long in a 99.6% positive market has an inflated Sharpe. Coefficient signs are economically motivated: wind_err_lag1 = -1.57 (more wind overperformance yesterday = below-trend today, because the system was long), solar_err_lag1 = -0.83 (same), wind_fc_d = -0.98 (higher expected wind supply = below trend), rebap_dev_lag1 = +0.30 (mild persistence in the deviation). All stable (cv < 1 on all four features). The look-ahead re-frame (excess vs level) is the core methodological point: always check the naive baseline before reporting directional accuracy on a structurally skewed target.

**Decision:** The excess-above-trend framing is correct and should be the model: it isolates the forecast-driven component of reBAP rather than the always-positive regime premium. Showing the naive alongside the model on the equity chart makes the distinction explicit. The 70% accuracy on excess direction is a real signal: renewable underperformance yesterday predicts above-trend reBAP today with consistent economic sign. Frontend: `ImbalanceSignalSection` with today's signal chip, 4-stat grid (model Sharpe, excess accuracy vs naive, naive Sharpe/pos-rate, max DD), dual equity curve, and a coefficient bar chart with economic-direction notes per feature.

**Artifacts:** `backend/analytics/imbalance.py` (`compute_rebap_signal`, `_coef_stability`, constants), `backend/app/schemas.py` (RebapSignal* models), `backend/app/main.py` (GET /api/imbalance/signal), `backend/tests/test_endpoints.py` (`test_imbalance_signal`), `frontend/src/lib/api.ts` (RebapSignalResponse types + `imbalanceSignal`), `frontend/src/routes/imbalance.tsx` (`ImbalanceSignalSection`). 108 tests passing. ROADMAP updated with Phases 56 (US ISO LMP) and 57 (nuclear * wind interaction) as future phases.

## 2026-06-27 - Phase 53: Does the residual actually revert? - the fade's premise, tested

**Tried:** Eleven phases (P42-P52) all trade the same bet - that the OOS nonlinear residual (actual DA price minus the hinge fair value) reverts to zero, so a contrarian fade on its rolling z-score earns - yet that premise was never tested directly. Built `compute_residual_mean_reversion`: it pulls each fundamental zone's genuinely-OOS nonlinear residual series (exposed `res_nl` from `_nonlinear_signal_pnl`) and runs three independent mean-reversion diagnostics from `quant_lib.meanrev` (core, no new dep): the OU/AR(1) fit (`fit_ou`) for the half-life = ln(2)/theta, the Lo-MacKinlay variance-ratio test (`variance_ratio_test`) for the random-walk null, and the variance-scaling Hurst exponent (`hurst_varscale`). A zone is flagged mean-reverting only on the conservative AND of all three (finite half-life, VR < 1 at p < 0.05, H < 0.5). `GET /api/spreads/residual-mean-reversion`.

**Found:** The premise holds decisively, and a methodological gotcha surfaced. All 5 zones' residuals are mean-reverting on every diagnostic: half-lives 1.2-2.7 trading days (BE/DE-LU/NL fastest at ~1.3d, FR slowest at 2.7d - consistent with nuclear-baseload FR pricing reverting more slowly), variance ratios 0.08-0.18 (all p < 0.05, so the random walk is rejected hard - a 20-day variance only ~8-18% of the random-walk expectation), and variance-scaling Hurst 0.01-0.05 (strongly anti-persistent). The gotcha: the rescaled-range (R/S) Hurst, my first choice, read 0.7-0.9 for every zone - spuriously "trending", flatly contradicting the OU and VR results - because R/S is badly upward-biased on short, fast-reverting series. Switching to the variance-scaling estimator (same logic family as the VR test) brought Hurst into agreement (~0.05). The ~1-3 day half-life is the quantitative justification for the arc's daily-rebalanced structure: the edge decays in days, so you must trade it daily.

**Decision:** A research arc this deep needs its foundation made explicit, not assumed - "is the thing you fade actually mean-reverting, and how fast?" is the first question under the P&L, and now the page answers it before the first backtest (the section sits directly above NonlinearBacktestSection so the argument reads premise -> fade -> P&L -> costs -> cross-zone -> regime -> portfolio). Requiring all three diagnostics to agree, and documenting why R/S was rejected for variance-scaling, keeps the verdict honest rather than cherry-picked. Frontend: `ResidualMeanReversionSection` with a pass-count banner, a per-zone table (half-life, VR, VR p-value, Hurst, verdict, each cell colour-coded to its mean-reversion threshold), and a footer that reads off the half-life as the implied holding horizon and states the R/S-vs-variance-scaling caveat.

**Artifacts:** `backend/analytics/fundamental.py` (`compute_residual_mean_reversion`, `res_nl`/`res_dates` exposed from `_nonlinear_signal_pnl`), `backend/app/schemas.py` (`ResidualMeanReversionRow`, `ResidualMeanReversionResponse`), `backend/app/main.py` (endpoint), `backend/tests/test_endpoints.py` (`test_spreads_residual_mean_reversion`), `frontend/src/lib/api.ts` (types + `spreadsResidualMeanReversion`), `frontend/src/routes/spreads.tsx` (`ResidualMeanReversionSection`). 107 tests passing; live endpoint verified (5/5 zones mean-reverting, half-lives 1.2-2.7d).

---

*Addendum (same session):* **Phase 54 - D-1 nuclear lag in enriched + GBM fair-value models.** Added `nuclear_lag1_gw` (previous day realised nuclear output, GW) to the gate-closure information set used by both the walk-forward enriched-OLS and the 21-day GBM re-fit on /spreads. Data source: `power_generation_actual tech='nuclear'` already populated by Phase 12's `entso-e-gen-full` fetcher - no new fetch step needed. `_build_nuclear_lag()` groups by zone/date, averages MW/1000 to GW, and shifts by 1 day within each zone so the D-1 actuals (published on ENTSO-E before market open) are the only look-ahead-free proxy in the gate-closure set. FR result: nuclear coef mean 0.0065 EUR/MWh per GW, cv=32 (near-zero and highly unstable - nuclear in FR is baseload, not the marginal setter), enriched RMSE improves 7.9% overall and 11.2% on low-wind days. DE-LU: coef exactly 0.0 (correct - Germany phased out April 2023, the column is 0 throughout the OOS window). The small positive mean for FR still earns a real RMSE gain because the cross-border flow effect of FR nuclear on neighbouring prices is absorbed by the model even when the direct marginal-plant channel is absent. GBM extended from 6 to 7 features (`nuclear_gw`); enriched-OLS `DEFERRED_FACTORS` cleared (was blocking nuclear as look-ahead - the D-1 lag resolves that). Frontend: enriched-model coef grid widened to 3 columns; live FR nuclear chip (GW, utilization %, vs 5yr DOY avg) added below the current-signal cards. 107 tests passing after fixture and assertion updates.

## 2026-06-27 - Phase 52: Block-bootstrap Sharpe CI completes the portfolio robustness triad

**Tried:** P51 added the deflated Sharpe (a point-estimate haircut for multiple testing) but left the third leg of the standard robustness triad - a sampling-error confidence interval - unstated. A desk reading "Sharpe 3.80" still wants to know how wide the band is, and an i.i.d. bootstrap would lie because the fade holds a position for days/weeks, so the daily P&L is autocorrelated. Added `_block_bootstrap_sharpe_ci`: a circular block bootstrap (block = 10 trading days, 2000 resamples, fixed seed for reproducibility) that resamples contiguous blocks to preserve the serial structure, then reports the 90% annualised-Sharpe CI and P(Sharpe > 0) for the OOS portfolio and for DE-LU alone. Pure numpy, no new dependency. Wired into the existing `significance` block of `compute_portfolio_backtest` (`bootstrap_portfolio_oos`, `bootstrap_de_lu`).

**Found:** The CI sharpens the diversification story the deflation started. The OOS portfolio's 90% Sharpe CI is **[2.67, 5.12]** - entirely above zero, with P(Sharpe > 0) = 100% - so even the pessimistic tail of the sampling distribution is a strong book. DE-LU alone is **[0.10, 2.25]**, barely clearing zero (P(Sharpe > 0) = 96.3%): its lower bound sits essentially at the no-skill line. The two diagnostics now agree from different angles - the single zone fails the multiple-testing haircut (DSR 0.22) *and* its bootstrap band reaches down to zero, while the portfolio survives both (DSR 0.99, CI floor 2.67).

**Decision:** Completing the triad (bootstrap CI + PSR + DSR, the same three tools as `quant_lib.validation.robustness_summary`) is the right way to close the capstone: a Sharpe with a CI, a multiple-testing benchmark, and a deflation probability is the full defensible claim, not a bare point estimate. The block bootstrap (not i.i.d.) is non-negotiable given the position persistence. Kept it integrated into the portfolio significance display rather than a new section - it is the same argument, better quantified. Frontend: the deflation strip now appends the bootstrap CIs inline, contrasting the portfolio's [2.67, 5.12] floor-above-zero against DE-LU's near-zero lower bound.

**Artifacts:** `backend/analytics/fundamental.py` (`_block_bootstrap_sharpe_ci`, `PORTFOLIO_BOOT_*`, bootstrap fields in the significance block), `backend/app/schemas.py` (`BootstrapSharpeCI`, extended `PortfolioSignificance`), `backend/app/main.py` (wiring), `backend/tests/test_endpoints.py` (CI assertions), `frontend/src/lib/api.ts` (types), `frontend/src/routes/spreads.tsx` (inline CI in the deflation strip). 106 tests passing; live endpoint verified (port CI [2.67, 5.12], DE-LU [0.10, 2.25]).

## 2026-06-27 - Phase 51: Harden the portfolio capstone (OOS rolling weights + deflated Sharpe)

**Tried:** P50's own caveat was the most pokeable number in the whole arc: the 5-zone portfolio's headline Sharpe 3.76 used inverse-vol weights set from *full-sample* volatility - an ex-post overlay on otherwise-OOS per-zone signals, which the docstring itself flagged as needing a rolling-weight version. Closed that gap and added the multiple-testing rigor the arc was missing. (1) **Rolling inverse-vol weights (genuinely OOS):** `compute_portfolio_backtest` now forms each day's weights from a trailing 252-day window of realised per-zone net P&L (min 20 obs), lagged one day so day t's weight uses only information through t-1, renormalised across the zones with enough history that day. This makes the *whole* portfolio out-of-sample - signals and construction. The full-sample book is retained as an ex-post reference (it is also what the Euler risk attribution and diversification ratio are computed on, since both need one static weight vector). (2) **Deflated Sharpe (Bailey & Lopez de Prado 2012):** PSR vs 0 and DSR vs the expected-max Sharpe from `PORTFOLIO_N_TRIALS=25` independent trials (5 fundamental zones x 5 model classes weighed over P42-P50), applied to the OOS portfolio and to DE-LU alone. Reimplemented PSR/DSR inline (pure numpy + scipy, already in the venv) rather than importing `quant_lib.validation.robustness`, whose package `__init__` pulls in sklearn (absent here, and the arc deliberately stayed sklearn-free). `GET /api/spreads/portfolio-backtest` gains `weighting_oos`, `portfolio_oos`, `significance{}`, and a `cum_portfolio_oos` equity line.

**Found:** The capstone hardens cleanly and the story gets *better*, not weaker. The OOS rolling-weight portfolio earns **Sharpe 3.80 - essentially identical to the ex-post 3.76** - so the headline was never a look-ahead-weighting artefact; genuine causal weighting reproduces it. The deflation is the sharper result: after a 25-trial haircut (selection-bias benchmark Sharpe 1.94), the OOS portfolio's **DSR is 0.993** - it clears the benchmark comfortably - while the single best zone **DE-LU alone deflates to 0.22** (its standalone Sharpe 1.16 sits *below* the 1.85 benchmark). The contrast is the lesson: diversification across the five low-correlation zone fades, not any one signal, is what survives multiple testing. A one-zone book would not.

**Decision:** Rolling, past-only weights are non-negotiable for a tradeable portfolio claim - "your 3.76 is ex-post weighted" is the first question an interviewer asks, and the honest answer is now on the page: the OOS number matches. Pairing it with a deflated Sharpe turns the arc's nine phases of model selection from an unstated liability into an explicit, auditable haircut - and the haircut is what makes the diversification case rigorous rather than asserted. Frontend: `PortfolioSection` now leads with the OOS Sharpe (ex-post shown as a subscript reference), a DSR headline card, a dedicated deflation strip contrasting the portfolio's surviving DSR against DE-LU's collapse, and a three-line equity chart (OOS / ex-post dashed / DE-LU). Honest caveat retained: the Euler decomposition still uses the static reference weights, since per-day risk attribution under time-varying weights is not single-valued.

**Artifacts:** `backend/analytics/fundamental.py` (`PORTFOLIO_ROLL_WIN/MIN/N_TRIALS`, `_per_obs_sharpe`, `_psr`, `_deflated_sharpe`, rolling-weight + significance block in `compute_portfolio_backtest`), `backend/app/schemas.py` (`DeflatedSharpe`, `PortfolioSignificance`, extended `PortfolioBacktestResponse`/`PortfolioEquityPoint`), `backend/app/main.py` (endpoint wiring), `backend/tests/test_endpoints.py` (extended `test_spreads_portfolio_backtest`), `frontend/src/lib/api.ts` (types), `frontend/src/routes/spreads.tsx` (`PortfolioSection`). 106 tests passing; live endpoint verified (OOS 3.80, DSR 0.99 vs DE-LU 0.22).
