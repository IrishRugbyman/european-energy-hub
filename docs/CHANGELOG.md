...
**Built:** New cross-market analysis quantifying the Nordic "hydrological balance" price fundamental. `analytics/hydro_balance.py` joins the weekly reservoir balance (storage vs 5yr same-week seasonal norm, in %) against each country's weekly mean day-ahead price (cross-zone mean of its bidding zones), then fits price ~ balance per country and reports the Pearson correlation, R², OLS slope (EUR/MWh per pp of fill), current standing, fitted price, and residual. Covers 11 hydro-reporting countries (NO SE FI ES PT FR IT AT CH RO GR) with both reservoir and price coverage. Two precomputed energy_hub.duckdb tables (`hydro_price_balance_summary`, `hydro_price_balance_series`, ~3yr weekly), `GET /api/hydro/price-balance` endpoint, and `HydroBalanceSection` on the /spreads "Market Structure" tab: country selector, 4 stat cards (corr/R², current balance, price-vs-fit residual, slope), a weekly balance-vs-price scatter with the OLS regression line, and a cross-country ranking table classifying each market as hydro-set / hydro-influenced / thermal-set. 135 backend tests (+1), 60 frontend tests.

**Findings:** The correlation magnitude grades how hydro-set each market is. IT (-0.69, R²=0.48), PT (-0.62), ES (-0.60) are the most reservoir-sensitive; NO (-0.47) is the classic Nordic hydro signal. Thermally/nuclear-set markets sit near zero: AT (-0.01), FR (-0.09), CH (+0.05). As of mid-June 2026 Norway's reservoir balance is -2.6% (price 59 EUR/MWh, 3.6 below the hydro-fitted level) while Spain sits +38.5% (brimming reservoirs, price 46 EUR/MWh, 16 above fit - a non-hydro driver lifting price). The residual isolates weeks where price is rich or cheap versus what reservoirs alone imply, flagging when a cold snap, outage, or fuel shock - not the hydro balance - is setting the margin. This completes the supply-side picture on /spreads, which was previously DE-LU thermal-only: the hydro-dominated Nordic/Iberian/Italian markets now have their dominant fundamental made explicit.
