...
## Phase 41 - French river temperature + nuclear curtailment risk tracker

**Why it matters:** When the Garonne, Rhone, Loire, or Moselle exceed thermal permit
thresholds (24-28°C depending on plant-specific environmental authorisation), EDF must
reduce or halt reactor output. Summer 2022: Rhone and Garonne stations triggered
curtailments of ~3 GW at peak crisis. This is a directly observable leading indicator
for FR nuclear output reduction, which in turn widens the FR-DE price spread.
**Alpha signal: river temp anomaly -> nuclear curtailment risk -> FR-DE spread widening.**

**Data source: Hub'Eau** (https://hubeau.eaufrance.fr) - BRGM/OFB free public API,
no key required. Returns daily water temperature measurements from ~4,500 French
gauging stations dating to the 1970s. Endpoint:
`GET /api/v1/temperature/chronique.json?code_station=<code>&date_debut_mesure=YYYY-MM-DD`

**Stations to track (8 core gauging stations near nuclear plants):**

| Station | River | Nuclear plant(s) nearby | Capacity at risk |
|---|---|---|---|
| 05230500 (Lamagistere) | Garonne | Golfech (1.4 GW) | ~1.4 GW |
| 06107000 (Givors) | Rhone | Bugey (1.8 GW), St-Alban (1.5 GW) | ~3.3 GW |
| 06117000 (Viviers) | Rhone | Cruas-Meysse (3.7 GW), Tricastin (3.6 GW) | ~7.3 GW |
| 06083000 (Lyon-Guillotiere) | Rhone | upstream of Bugey | reference |
| A6240150 (Dampierre) | Loire | Dampierre (3.7 GW) | ~3.7 GW |
| A4430850 (Belleville) | Loire | Belleville (2.6 GW) | ~2.6 GW |
| K1763100 (Cattenom area) | Moselle | Cattenom (5.4 GW) | ~5.4 GW |
| 03415200 (Chooz) | Meuse | Chooz (2.9 GW) | ~2.9 GW |

**Thermal permit thresholds (approximate - vary by station and season):**
- Normal operating limit: 24°C (many plants)
- Summer derogation limit: 27-28°C (granted by ASN in heat waves)
- Alert threshold for this tracker: >22°C (2°C buffer = early warning)

**Build plan:**

### Backend (market-data layer)
- [ ] `market-data/db.py`: add `river_temp` table
  ```sql
  CREATE TABLE IF NOT EXISTS river_temp (
      obs_date DATE NOT NULL,
      station_code VARCHAR(20) NOT NULL,
      river VARCHAR(50),
      station_name VARCHAR(100),
      temp_c REAL,
      PRIMARY KEY (obs_date, station_code)
  )
  ```
- [ ] `market-data/fetchers/hubeau.py`: Hub'Eau chronique fetcher
  - Fetches `/api/v1/temperature/chronique.json` for each station
  - Incremental from latest stored date (default from 2015-01-01 for 5yr baseline)
  - Handles pagination (page/size), stores daily mean if multiple obs/day
  - No API key required
- [ ] `market-data/config.py`: add `HUBEAU_STATIONS` list (station codes + metadata)
- [ ] `market-data/ingest.py`: add `hubeau` CLI command

### Backend (energy app)
- [ ] `energy/backend/analytics/river_temp.py`: analytics module
  - `build_river_temp_tables(db)`: queries energy_hub.duckdb after refresh writes temp data
  - Computes: `temp_latest` (most recent reading per station), `temp_trend` (last 90 days
    per station), `temp_seasonal` (5yr avg/min/max at same DOY per station)
  - Derives: `alert_level` = 'normal' | 'watch' (>22°C) | 'warning' (>24°C) | 'critical' (>26°C)
  - Computes: `capacity_at_risk_mw` = sum of plant capacity for stations above 22°C threshold
- [ ] `energy/backend/scripts/refresh.py`: add `hubeau` to ingest list + call river_temp build
- [ ] `energy/backend/app/schemas.py`: RiverTempRow, RiverTempResponse
- [ ] `energy/backend/app/main.py`: `GET /api/generation/river-temp` endpoint

### Frontend
- [ ] `api.ts`: add `RiverTempRow`, `RiverTempResponse`, `api.genRiverTemp()`
- [ ] `generation.tsx`: add `RiverTempSection` component in `#gen-nuclear` section
  - Alert strip: "X GW at watch/warning (river temps above 22°C)" if any station triggered
  - Station list: temp gauge per station (current vs threshold vs 5yr avg), river + plant label
  - 90-day trend chart for each river (Garonne / Rhone / Loire / Moselle in separate lines)
  - Annotation: permit threshold line at 24°C, warning at 26°C

### quant_lib.features integration (for algo trading)
- [ ] `quant_lib/features/river_temp.py`: `build_river_temp_features(conn, zone='FR', lookback_days=30)`
  - Returns DataFrame with columns: `temp_above_22_days`, `temp_above_24_days`, `max_temp_7d`,
    `temp_anomaly_vs_avg5`, `capacity_at_risk_mw`, `capacity_at_risk_pct`
  - These become ML features for predicting FR nuclear output -> FR-DE spread
  - Reads from PostgreSQL `river_temp` table via market-data loaders
- [ ] Add to quant_lib `features` subpackage __init__ exports
- [ ] Tests in `quant-lib/tests/test_river_temp_features.py`

### Definition of done
- [ ] Hub'Eau fetcher backfills to 2015-01-01 for all 8 stations (5yr+ seasonal baseline)
- [ ] Twice-daily refresh includes river temp ingest
- [ ] `/api/generation/river-temp` returns current alert level + trend for all stations
- [ ] Frontend: alert strip visible on /generation nuclear section when any station >22°C
- [ ] quant_lib.features: `build_river_temp_features()` callable from research notebooks
- [ ] 88+ tests passing
